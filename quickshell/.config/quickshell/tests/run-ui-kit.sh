...
test_dir=$(mktemp -d "${TMPDIR:-/tmp}/quickshell-ui.XXXXXX")
...
if ! env XDG_RUNTIME_DIR="$test_dir/runtime" XDG_STATE_HOME="$test_dir/state" \
    ...
    timeout 15s qs -p "$test_dir/config/ui-kit-tests.qml" --no-color > "$test_dir/log" 2>&1; then
    cat "$test_dir/log"
    exit 1
fi
cat "$test_dir/log"
if rg -q 'FAIL:|ERROR' "$test_dir/log"; then exit 1; fi
rg -q 'PASS: UI kit checks' "$test_dir/log"
